...
(defmacro %aref1-body ((vector-var index-var data-var offset-var) &body body)
  (destructuring-bind (simple-form complex-form) body
    (let ((typecode (gensym "TYPECODE")))
      `(let* ((,typecode (typecode ,vector-var)))
         (declare (type (unsigned-byte 8) ,typecode))
         (cond ((or (= ,typecode target::subtag-simple-vector)
                    (>= (the (unsigned-byte 8) (ivector-typecode-p ,typecode))
                        target::min-cl-ivector-subtag))
                ,simple-form)
               ((= ,typecode target::subtag-arrayH)
                (%err-disp $XNDIMS ,vector-var 1))
               ((/= ,typecode target::subtag-vectorH)
                (report-bad-arg ,vector-var 'array))
               ((not (typep ,index-var 'fixnum))
                (report-bad-arg ,index-var 'fixnum))
               ((not (and (>= (the fixnum ,index-var) 0)
                          (< (the fixnum ,index-var)
                             (the fixnum (%svref ,vector-var target::vectorH.physsize-cell)))))
                (%err-disp $XARROOB ,index-var ,vector-var))
               (t
                (multiple-value-bind (,data-var ,offset-var)
                    (%array-header-data-and-offset ,vector-var)
                  ,complex-form)))))))

(defun %aref1 (vector index)
  (%aref1-body (vector index data offset)
    (uvref vector index)
    (uvref data (+ offset index))))

(defun %aset1 (vector index new)
  (%aref1-body (vector index data offset)
    (setf (uvref vector index) new)
    (setf (uvref data (+ offset index)) new)))
