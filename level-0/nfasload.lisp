...
(defun %simple-fasl-read-buffer (s)
  (let* ((fd (faslstate.faslfd s))
    ...
    (let* ((n (fd-read fd bufptr ccl-fasl:+fasl-buffer-length+)))
      (declare (fixnum n))
      (if (> n 0)
...
(defun %fasl-dispatch (s op)
  (declare (fixnum op))
  (flet ((split-operation (op)
           (values (logand op (lognot (ash 1 ccl-fasl:+fasl-epush-bit+)))
                   (logbitp ccl-fasl:+fasl-epush-bit+ op))))
    (multiple-value-bind (operation epushp) (split-operation op)
      (setf (faslstate.faslepush s) epushp)
      (funcall (svref (faslstate.fasldispatch s) operation) s))))
...
(defun %fasl-check-header (s)
  (let* ((signature (%fasl-read-word s)))
    ...
    (if (= signature ccl-fasl:+fasl-file-id+)
      (values t 0)
      (values nil $xnotfasl))))
...
(defun %fasload (string &optional (table *fasl-dispatch-table*))
  ...
  (let* ((s (%make-faslstate)))
    ...
    (%stack-block ((buffer (+ target::node-size ccl-fasl:+fasl-buffer-length+)))
