      ...
      (cond ((keywordp symbol)
            ...
            ((null package)
             (when (or *print-readably* *print-gensym*)
               ...
               (stream-write-char stream #\:)))
            (t
             (multiple-value-bind (s flag) (find-symbol name *package*)
               (unless (and flag (eq s symbol))
                 (multiple-value-setq (s flag) (find-symbol name package))
                 (unless (and flag (eq s symbol))
                   (%write-string "#|symbol not found in home package!!|#"
                                  stream))
                 (let ((local-nickname (and (package-%local-nicknames *package*)
                                            (car (rassoc package (package-%local-nicknames *package*))))))
                   (if local-nickname
                     (write-pname local-nickname case stream)
    ...
    (let ((tail (%cdr dimensions))
      ...
      (do* ((o offset (%i+ o step))
        ...
        (declare (type fixnum o) (type fixnum i) (type fixnum limit)
                 ...
                 (type fixnum level-1))
        (cond ((eql i limit)
               ;; Need to test first if limit is reached (and return)
