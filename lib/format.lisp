            ...
            (when segments
              (let* ((padsegs (+ (if (or colon (= numsegs 1)) 1 0)
                  ...
                  (with-format-parameters special-parms ((spare 0)
                    ...
                    (let ((pos (column stream)))
                      (when (> (+ pos width spare) linel)
                        (stream-write-string stream special-arg)))))
                (do ((segs segments (cdr segs))
                  ...
                  (stream-write-string stream (car segs)))))))))))
          ...
          (let ((w (multiple-value-list
                ...
                (f (multiple-value-list
                    (flonum-to-digits x (- (+ 1 (or fmin 0)
                                              (if scale scale 0)))))))
            (if (or (>= (length (cadr w)) (length (cadr f)))
                    (>= (length (cadr f)) width))
              (values-list w)
              (values-list f)))
          (flonum-to-digits x)))
...
(defun format-print-cardinal (stream n)
  (cond ((minusp n)
         (stream-write-string stream "negative ")
         (format-print-cardinal-aux stream (- n) 0 n))
        ((zerop n)
         (stream-write-string stream "zero"))
...
(defun format-print-cardinal-aux (stream n period err)
  (multiple-value-bind (beyond here) (truncate n 1000)
    ...
    (unless (zerop here)
      ...
      (stream-write-string stream (svref (cardinal-periods) period)))))
  ...
  (let ((number (abs n)))
    (multiple-value-bind (top bot) (truncate number 100)
      ...
      (multiple-value-bind (tens ones) (truncate bot 10)
        (cond ((= bot 12) (stream-write-string stream "twelfth"))
              ((= tens 1)
               (stream-write-string stream (svref (cardinal-teens) ones));;;RAD
               (stream-write-string stream "th"))
              ((and (zerop tens) (plusp ones))
               (stream-write-string stream (svref (ordinal-ones) ones)))
              ((and (zerop ones)(plusp tens))
               (stream-write-string stream (svref (ordinal-tens) tens)))
              ((plusp bot)
               (stream-write-string stream (svref (cardinal-tens) tens))
               (write-char #\- stream)
               (stream-write-string stream (svref (ordinal-ones) ones)))
              ((plusp number) (write-string "th" stream :start  0 :end 2))
              (t (stream-write-string stream "zeroth")))))))
          ...
          (let ((strlen (length string)))
            ...
            (when e 
              ...
              (dotimes (i (- e (length estr)))
                (declare (fixnum i))
                (write-char #\0 stream)))
            (stream-write-string stream estr))))))
#|
  ...
  (with-format-parameters parms ((d 2) (n 1) (w 0) (pad #\space))
    (let* ((number (float (pop-format-arg)))
      ...
      (multiple-value-bind (str before-pt after-pt) (flonum-to-string number nil d)
        ...
        (when colon (stream-write-string stream signstr))
        (dotimes (i spaceleft) (write-char pad stream))
        (unless colon (stream-write-string stream signstr))
        (cond
         ((> before-pt 0)
